...
export default({}) => {
  const web3 = new Web3('https://mainnet.infura.io/v3/4e4cf0a867c84ff382e22709aadc1e50')
  ...
  useEffect(() => {
    ...
    if (!mounted) {
      ...
      fetch('https://api.etherscan.io/api?module=stats&action=ethprice').then(res => res.json()).then(r => {setEthPrice(r.result.ethusd)})
    }
  })
    ...
    <>
      ...
      <Row>
        <Col lg={12} md={12} sm={12}>
          <PNKSoldText>{String(Number(fromWei(purchaseAmount)).toFixed(0)).replace(/\B(?=(\d{3})+(?!\d))/g, ",")} Sold</PNKSoldText>
        </Col>
        <Col lg={12} md={12} sm={12}>
          <RemainingText>{String(Number(fromWei(toBN(saleTotal).sub(purchaseAmount))).toFixed(0)).replace(/\B(?=(\d{3})+(?!\d))/g, ",")} Remaining</RemainingText>
        </Col>
      </Row>
    </>
  )
