...
const Latest = ({ language }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await fetch("https://forum.kleros.io/posts.json", {
        method: "GET"
  ...
  return (
    <StyledLatest>
      <StyledHeading>
        {<FormattedMessage id="home.latest.heading" />}
      </StyledHeading>
      <Row>
        <Col lg={12}>
          <NewsBox
            icon={Twitter}
            date={"Dec 12, 2019"}
            preview={"balh blah blah"}
            linkText={"Go to Twitter"}
            link={"https://twitter.com/Kleros_io"}
          />
        </Col>
        <Col lg={12}>
          <NewsBox
