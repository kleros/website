...
export default function ca_sketch() {
	...
	class Dot {
		...
		move() {
			if (!this.transporting) {
				this.position.x = this.anchor.x + this.p.cos(time * this.direction + this.startAngle) * this.radius;
				this.position.y = this.anchor.y + this.p.sin(time * this.direction + this.startAngle) * this.radius;
			} else {
		...
		transport(newAnchor) {
			...
			this.transportLoc.x = this.anchor.x + this.p.cos(time * this.direction + this.startAngle) * this.radius;
			this.transportLoc.y = this.anchor.y + this.p.sin(time * this.direction + this.startAngle) * this.radius;
	...
	class Shape {
		...
		changeImage(image = this.image) {
			...
			this.graphic.copy(image, 0, 0, image.width, image.height, parseInt(this.graphic.width / 2 - image.width / 2), parseInt(this.graphic.height / 2 - image.height / 2), image.width, image.height);
			...
			for (let i = 0; i < numOfDots; i++) {
				...
				if (pixel < 255) {
					...
					if (freeDots.length === 0) {
						...
						const startPosition = this.p.createVector(anchor.x + radius, anchor.y);
						...
						const dot = new Dot(anchor, startPosition, radius, direction, startAngle, this.p);
						this.dots.push(dot);
					} else {
		...
		static checkForConnect(dotA, dotB, numOfConnections, p) {
			...
			const distance = p.dist(dotA.position.x, dotA.position.y, dotB.position.x, dotB.position.y);
			if (distance > 0 && distance < maxConnectionLength) {
				...
				p.line(dotA.position.x, dotA.position.y, dotB.position.x, dotB.position.y);
				newNumOfConnections++;
			}
							...
							for (let addRow = -1; addRow <= 1; addRow++) {
								for (let j = 0; j < this.grid.dots[col + addCol][row + addRow].length; j++) {
									if (!(addCol === 0 && addRow === 0) || i !== j) {
										numOfConnections = Shape.checkForConnect(this.grid.dots[col][row][i], this.grid.dots[col + addCol][row + addRow][j], numOfConnections, this.p);
									}
								}
							}
						}
	...
	$(document).ready(function(){
		...
		var sketch = function(p) {
			...
			p.setup = function() {
				...
				const canvas = p.createCanvas(theContainer.width(), theContainer.height());
				canvas.parent(theContainer.get(0));
				//p.background(20);
				p.clear();
				numOfDots = p.max(theWidth, theHeight) * 6;
				shape = new Shape(theWidth, theHeight, theWidth / 2, theHeight / 2, input, p);
		...
		var theSketch;
		$('.ca_illustration_holder > img').one("load", function() {
