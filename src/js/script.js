	...
	$(document).ready(function(){
		...
		$('.ca_header nav .ca_hamburger').click(function(){
			if($('.ca_header').hasClass('ca_menu_opened'))
				...
				$('.ca_header').addClass('ca_menu_opened');
			return false;
		});
		...
		$('.ca_open_video_popup').click(function(){
			...
			if(video_id != false) {
				$('.ca_video_holder').find('iframe').attr('src', 'https://www.youtube.com/embed/' + video_id);
		...
		$('.ca_video_popup a.ca_close').click(function(){
			...
			$('.ca_video_holder').find('iframe').attr('src', 'https://www.youtube.com/embed/');
			return false;
		})
				...
				animations: [ // Array of actions attached to the detector of the given element
					{
						action: function(params) {
							if($('.ca_paralax_background_illustration').length) {
								...
								var top3 = (1 - val) * 0.07;
								var top4 = (1 - val) * 0.03;
								params.element.find('.ca_paralax_background_illustration span').eq(0).css({//+
									marginTop: top1 + 'em',
								});
								params.element.find('.ca_paralax_background_illustration span').eq(1).css({
									transform: 'scale(' + (1 - top2) + ', ' + (1 - top2) + ')',
								});
								params.element.find('.ca_paralax_background_illustration span').eq(2).css({//+
									marginTop: top3 + 'em',
								});
								params.element.find('.ca_paralax_background_illustration span').eq(3).css({
									marginTop: top4 + 'em',
									transform: 'scale(' + (1 + top4 * 0.03) + ', ' + (1 + top4 * 0.03) + ')',
								});
							}
						},
						probe: 0,
